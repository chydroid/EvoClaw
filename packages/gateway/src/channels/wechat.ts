...
export class WeChatAdapter implements ChannelAdapter {
  ...
  async handleOfficialMessage(
    ...
    const msg: ChannelMessage = {
      ...
      text: "",
      timestamp: (Number.isFinite(Number(parsed.CreateTime)) ? new Date(Number(parsed.CreateTime) * 1000) : new Date()).toISOString(),
      isDirect: true,
      isGroup: false,
  ...
  async handleWeComWebhook(
    ...
    const msg: ChannelMessage = {
      ...
      text: "",
      timestamp: (Number.isFinite(Number(body.CreateTime)) ? new Date(Number(body.CreateTime) * 1000) : new Date()).toISOString(),
      isDirect: true,
      isGroup: false,
      raw: body,
    };

...
const CAPABILITIES: CapabilityDef[] = [
  ...
  // ── v0.70 新增：一线 AI Agent 能力对齐 ──
  {
    ...
    version: "v0.70",
    icon: "🌿",
    description: "封装 git status/diff/log/blame/show/branch/add/commit/push/pull/checkout/merge/rebase 全套命令，内置危险命令拦截（force push、reset --hard、clean -f 等），保障版本操作安全。",
    descriptionEn: "Wraps the full git command suite (status/diff/log/blame/show/branch/add/commit/push/pull/checkout/merge/rebase) with dangerous command interception (force push, reset --hard, clean -f) for safe version control operations.",
    module: "@evoclaw/agent",
    tags: ["版本控制", "安全"],
    apiPath: "/api/agent/git-stats",
    statusKey: "gitOperations",
  },
  {
    ...
    version: "v0.70",
    icon: "🔍",
    description: "多语言代码符号解析与重命名（无外部依赖），支持定义/引用查找与跨文件重命名，为重构提供精确语义信息。",
    descriptionEn: "Multi-language code symbol parsing and renaming with no external dependencies; supports definition/reference lookup and cross-file renaming for precise refactor semantics.",
    module: "@evoclaw/agent",
    tags: ["代码理解", "重构"],
    apiPath: "/api/agent/code-intel-stats",
    statusKey: "codeIntelligence",
  },
  {
    id: "apply-patch",
    name: "SEARCH/REPLACE 补丁应用",
    nameEn: "Apply Patch",
    version: "v0.70",
    icon: "🩹",
    description: "4-pass 匹配策略（exact → normalized → fuzzy → line-range）+ 路径越界检查，安全应用 LLM 生成的 SEARCH/REPLACE 补丁。",
    descriptionEn: "4-pass matching (exact → normalized → fuzzy → line-range) with path traversal guard for safely applying LLM-generated SEARCH/REPLACE patches.",
    module: "@evoclaw/agent",
    tags: ["代码修改", "安全"],
    apiPath: "/api/agent/apply-patch-stats",
    statusKey: "applyPatch",
  },
  {
    id: "vision-analyzer",
    name: "VLM 视觉分析",
    nameEn: "Vision Analyzer",
    version: "v0.70",
    icon: "👁️",
    description: "VLM 视觉分析能力（屏幕描述/UI 检测/元素查找/图片对比），LRU + TTL 缓存 + in-flight 去重，降低重复调用成本。",
    descriptionEn: "VLM visual analysis (screen description / UI detection / element lookup / image diff) with LRU + TTL cache and in-flight dedup to reduce repeated call cost.",
    module: "@evoclaw/agent",
    tags: ["视觉", "性能"],
    apiPath: "/api/agent/vision-stats",
    statusKey: "visionAnalyzer",
  },
  {
    id: "batch-executor",
    name: "批量并发执行",
    nameEn: "Batch Executor",
    version: "v0.70",
    icon: "🗂️",
    description: "信号量并发控制 + 滑动窗口限速 + DAG 拓扑排序 + 环检测 + 指数退避重试，安全高效执行批量任务。",
    descriptionEn: "Semaphore concurrency control + sliding-window rate limit + DAG topological sort + cycle detection + exponential backoff for safe and efficient batch task execution.",
    module: "@evoclaw/agent",
    tags: ["并发", "可靠性"],
    apiPath: "/api/agent/batch-stats",
    statusKey: "batchExecutor",
  },
  {
    id: "workflow-engine",
    name: "DAG 工作流引擎",
    ...
    version: "v0.70",
    icon: "🔀",
    description: "DAG 工作流引擎支持条件分支、节点超时与重试、checkpoint 持久化与 resume，长任务可断点续跑。",
    descriptionEn: "DAG workflow engine with conditional branching, node timeout/retry, checkpoint persistence and resume for long-running task continuation.",
    module: "@evoclaw/agent",
    tags: ["工作流", "可靠性"],
    apiPath: "/api/agent/workflow-stats",
    statusKey: "workflowEngine",
  },
  {
    id: "session-checkpoint",
    name: "会话检查点 + DLQ 批量重试",
    nameEn: "Session Checkpoint & DLQ",
    version: "v0.70",
    icon: "🔖",
    description: "会话检查点（save/load/list/delete/diff）+ 死信队列批量重试，失败隔离机制确保单点失败不影响整体。",
    descriptionEn: "Session checkpoint (save/load/list/delete/diff) + dead-letter queue batch retry with failure isolation ensuring single-point failures don't affect the whole.",
    module: "@evoclaw/agent",
    tags: ["可靠性", "状态恢复"],
    apiPath: "/api/agent/checkpoint-stats",
    statusKey: "sessionCheckpoint",
  },
];

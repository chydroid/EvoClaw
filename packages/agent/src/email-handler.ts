...
export async function detectAndConfigureEmailAccount(
  ...
  console.log(`[EmailHandler] Detected email account configuration: ${email.replace(/(.{2}).*(@.*)/, "$1****$2")}, password length: ${password.length}`);
  ...
  if (taskClassifier) {
    try {
      ...
      console.log(`[EmailHandler] Email config intent classification: ${result.primaryCategory} (confidence: ${result.confidence})`);
    } catch (classifyErr) {
      console.warn(`[EmailHandler] Email intent classification failed: ${classifyErr instanceof Error ? classifyErr.message : String(classifyErr)}`);
    }
  }
...
export async function handleEmailOperation(
  ...
  if (deps.registeredTools.has("email_list_inbox")) {
    try {
      ...
      console.warn(`[EmailHandler] Failed to get email inbox: ${inboxErr instanceof Error ? inboxErr.message : String(inboxErr)}`);
    }
  }

...
export async function handleSkillInstall(
  ...
  try {
    ...
    if (/https?:\/\//i.test(message)) {
      console.log(`[AgentModelExecutor] Skill install request contains URLs but extraction failed, falling through to LLM`);
      return null;
    }
...
export async function downloadAndExtractSkill(deps: SkillInstallerDeps, url: string): Promise<DownloadAndExtractResult> {
  ...
  // ── Step 1: Download ──
  console.log(`[AgentModelExecutor] Downloading skill from: ${url}`);
  try {
  ...
  console.log(`[AgentModelExecutor] Downloaded ${fileSize} bytes to ${zipPath}`);

  // ── Step 2: Extract ──
  console.log(`[AgentModelExecutor] Extracting skill to: ${skillsDir}`);
  try {
  ...
  console.log(`[AgentModelExecutor] Found SKILL.md at: ${skillMdPath}`);
...
export async function extractZip(zipPath: string, targetDir: string): Promise<void> {
  ...
  try {
    ...
    console.warn(`[AgentModelExecutor] tar extraction failed: ${(err as Error).message}`);
  }
  ...
  if (process.platform === "win32") {
    try {
      ...
      console.warn(`[AgentModelExecutor] PowerShell Expand-Archive failed: ${(err as Error).message}`);
    }
  }
  ...
  if (process.platform !== "win32") {
    try {
      ...
      console.warn(`[AgentModelExecutor] unzip failed: ${(err as Error).message}`);
    }
  }
  ...
  try {
    ...
    console.warn(`[AgentModelExecutor] adm-zip not available`);
  }

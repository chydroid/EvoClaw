...
export async function executeWithTimeout<T>(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const delayMs = Math.min(Math.pow(2, attempt) * 100, maxBackoffMs);
      await new Promise((r) => {
...
export class EvolutionProposer {
  ...
  private async generateCandidate(
    ...
    if (req.type === "code_patch" || req.type === "skill_update") {
      ...
      artifacts.push({
        name: `${req.type}_${randomUUID().slice(0, 8)}`,
        language: "typescript",
        source: improvement,
        tests: testCode,
        dependencies: [],
      ...
      codeChanges.push({
        filePath: `improvements/${req.type}_${randomUUID().slice(0, 8)}.ts`,
    ...
    if (req.type === "new_skill") {
      ...
      if (skillCode) {
        artifacts.push({
          name: `new_skill_${randomUUID().slice(0, 8)}`,
          language: "typescript",
          source: skillCode,
    ...
    if (req.type === "config_change") {
      codeChanges.push({
        filePath: `config/evolution_${randomUUID().slice(0, 8)}.json`,
